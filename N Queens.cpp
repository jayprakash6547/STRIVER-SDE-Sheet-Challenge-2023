...
bool isPossible(vector<vector<int>>& res,int n,int i,int j){
    for(int col=0;col<n;col++){
        if(res[i][col]==1) return false;
    }
    for(int row=0;row<n;row++){
        if(res[row][j]==1) return false;
    }
...
void place_N_queens(vector<vector<int>>& res,int n,int i,int j,vector<vector<int>>& ans){
     if(i==n){
        addBoard(res, ans);
        return;
     }
     for(int col=j;col<n;col++){
         if(isPossible(res,n,i,col)){
             res[i][col]=1;
             place_N_queens(res,n, i+1,0,ans);
             res[i][col]=0;
